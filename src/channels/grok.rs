...
impl Grok_ {
  pub fn new(field: String, pattern: String, custom: Option<&Vec<Value>>) -> Option<Self> {
    ...
    Some(Grok_ { modifier: Mutation { field }, compiled_pattern: compiled })
  }
  ...
  fn compile(pattern: String, custom_patterns: Option<Vec<(String, String)>>) -> Result<Pattern, Error> {
    ...
    grok.insert_definition("QUOTEDSTRING",
                       r#"(?>(?<!\\)(?>"(?>\\.|[^\\"]+)+"|""|(?>'(?>\\.|[^\\']+)+')|''|(?>`(?>\\.|[^\\`]+)+`)|``))"#);
    ...
    grok.insert_definition("HOSTNAME",
    ...
    grok.insert_definition("TIMESTAMP_ISO8601",
    ...
    grok.insert_definition("SYSLOGBASE",
    ...
    let pattern = grok.compile(pattern.as_str(), false).map_err(Error::GrokError)?;
...
impl Mutator for Grok_ {
  fn mutate(&self, v: &mut Map<String, Value>) -> Result<(), Error> {
    let value = v.get(&self.modifier.field).ok_or_else(|| Error::FieldNotFoundInJSON(self.modifier.field.clone()))?;
    ...
    let matches = self.compiled_pattern.match_against(incoming_value.as_str()).ok_or(Error::GrokNoMatches)?;
