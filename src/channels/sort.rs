...
fn maybe_reverse<T: Into<Value> + Ord>(descending: bool, res: impl Iterator<Item = T> + DoubleEndedIterator)
                                       -> Vec<Value> {
  if descending {
...
impl Mutator for Sort {
  fn mutate(&self, v: &mut Map<String, Value>) -> Result<(), Error> {
    let value =
      v.get(&self.modifier.field).ok_or_else(|| Error::FieldNotFoundInJSON(self.modifier.field.to_string()))?;
    ...
    let first_item = array.get(0).ok_or_else(|| Error::EmptyArray(self.modifier.field.to_string()))?;

    match first_item {
      Value::String(_) => {
        let res = array.iter().filter_map(|x: &Value| {
                                match x {
  ...
  fn mutator_type(&self) -> MutatorType { MutatorType::Sort }
}
